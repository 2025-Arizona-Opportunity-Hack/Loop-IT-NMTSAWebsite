...
export function Input({ label, error, required, className = '', ...props }: InputProps) {
  return (
    <div className="space-y-2">
      ...
      <input
        className={`w-full px-4 py-3 border rounded-xl focus:outline-none focus:ring-2 focus:ring-nmtsa-500 bg-white transition-all ${
...
export function Textarea({ label, error, required, className = '', ...props }: TextareaProps) {
  return (
    <div className="space-y-2">
      ...
      <textarea
        className={`w-full px-4 py-3 border rounded-xl focus:outline-none focus:ring-2 focus:ring-nmtsa-500 bg-white resize-none transition-all ${
          error ? 'border-red-500' : 'border-gray-200'
        } ${className}`}
        {...props}
      />
...
export function FormWrapper<T extends z.ZodType<any, any>>({
  ...
  return (
    <form onSubmit={form.handleSubmit(handleSubmit)} className={`space-y-6 ${className}`}>
      ...
      {isSuccess && (
        <div className="p-4 bg-green-50 border border-green-200 rounded-xl flex items-start gap-3 animate-in slide-in-from-top">
      ...
      <button
        ...
        className="w-full bg-gradient-to-r from-nmtsa-500 to-nmtsa-600 text-white font-semibold px-8 py-4 rounded-full hover:shadow-xl transition-all duration-300 hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none flex items-center justify-center gap-2"
      >
        {isSubmitting ? (
          <>
...
export function FormSection({ title, description, icon, children, className = '' }: FormSectionProps) {
  return (
    <div className={`bg-gradient-to-r from-gray-50 to-gray-100 p-6 rounded-xl ${className}`}>
      <div className="mb-4">
